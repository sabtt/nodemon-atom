...
NodemonAtomCommands = require '../nodemon-atom-commands'
fuzzy = require('../models/fuzzy').filter

module.exports =
class NodemonAtomMenu extends SelectListView

  initialize: ->
    super
    @addClass('nodemon-palette')
    @toggle()
  ...
  show: ->
    ...
    NodemonAtomCommands()
      .then (commands) =>
        ...
        @focusFilterEditor()
      # .catch (err) =>
      #   (commands = []).push { name: 'nodemon-atom:error', description: 'There was an error', func: -> false }
      #   @setItems(commands)
      #   @panel.show()
      #   @focusFilterEditor()
